...
| `tradeautonom-v3` (multi-user) | 8005 | `v3` | `/volume1/docker/tradeautonom-v3` | `deploy/v3/deploy.sh` |
| `ta-user-*` (user containers) | 9001–9003 | `v3` | managed by orchestrator | via orchestrator |
...
- This directory is shared by V3 + all user containers (Ports 8005, 9001, 9002, 9003)
- Uvicorn `--reload` detects changes → auto-restarts Python process within ~3 seconds
- Vault sessions persist — no manual unlock needed

**Safety:**
- Checks all containers (ports 8005, 9001, 9002, 9003) for active bots before deploying
...
- Only affects containers sharing the `/volume1/docker/tradeautonom/app/` mount
...
```bash
./deploy.sh config.py engine.py state_machine.py
```

### Frontend + Backend change
```bash
./deploy.sh config.py engine.py state_machine.py   # 1. Hot-deploy Python
./deploy/cloudflare/deploy.sh                       # 2. Build + deploy frontend to CF Worker
```
...
```bash
./deploy/v3/deploy.sh    # Full Docker rebuild required
```
